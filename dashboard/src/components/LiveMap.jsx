...
// Custom tactical icons using DivIcon inline SVGs to avoid CORS / loading issues
...
function MapRecenter({ coords, alerts }) {
  const map = useMap();

  useEffect(() => {
    if (alerts && alerts.length > 0) {
      const validCoords = alerts.filter(a => a.lat && a.lng).map(a => [a.lat, a.lng]);
...
export default function LiveMap({ alerts = [] }) {
  const [nodeCenter, setNodeCenter] = useState([CURRENT_NODE_LOCATION.lat, CURRENT_NODE_LOCATION.lng]);
  const [locStatus, setLocStatus] = useState(CURRENT_NODE_LOCATION.cameraName);
    ...
    if ('geolocation' in navigator) {
      navigator.geolocation.getCurrentPosition(
        (pos) => {
          ...
          setLocStatus('LIVE GPS LOCK ACTIVE');
        },
        (err) => {
          console.warn('Geolocation blocked or unavailable:', err);
          setLocStatus(`${CURRENT_NODE_LOCATION.cameraName} (STATIC)`);
        },
    ...
    <div className="w-full h-full min-h-[350px] rounded-xl overflow-hidden relative border border-slate-800 bg-slate-950 flex flex-col">
      ...
      <div className="flex-1 w-full h-full relative">
        ...
        >
          ...
          <MapRecenter coords={nodeCenter} alerts={alerts} />

          {/* Camera Node Marker */}
          <Marker position={nodeCenter} icon={cameraIcon}>
            <Popup>
              <div className="text-xs font-mono font-bold text-slate-800">
                📹 {CURRENT_NODE_LOCATION.cameraName}<br />
                <span className="text-emerald-600">PRIMARY NODE ACTIVE</span>
              </div>
            </Popup>
          </Marker>
